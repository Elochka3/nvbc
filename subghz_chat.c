...
static void render_callback(Canvas* canvas, void* model) {
    ...
    canvas_draw_str(canvas, 2, 12, "Chat Debug Mode v1.5");
...
static void send_radio_packet(ChatApp* app) {
    // ВРЕМЕННО: Только визуализация, чтобы проверить стабильность
    with_view_model(app->main_view, ChatModel* m, {
        strncpy(m->last_rx_msg, "TX: Data Sent", 63);
    }, true);
    
    // Попытка "тихого" захвата радиомодуля (без direct_tx)
    ...
    if(event->type == InputTypeShort) {
        if(event->key == InputKeyOk) {
            ...
            with_view_model(app->main_view, ChatModel * m, {
                ...
                // Прямая отправка команды пинам антенны (0 или 1)
