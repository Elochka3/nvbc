...
// Структура только для данных экрана
typedef struct {
    char last_rx_msg[64];
    bool is_external;
} ChatModel;
...
// Коллбэк отрисовки (теперь через Model)
...
static bool input_callback(InputEvent* event, void* ctx) {
    ...
    if(event->type == InputTypeShort) {
        if(event->key == InputKeyOk) {
            ...
            // Безопасно меняем данные внутри модели
            with_view_model(app->main_view, ChatModel * m, { m->is_external = !m->is_external; }, true);
            return true;
        }
    }
    return false;
...
int32_t subghz_chat_app(void* p) {
    ...
    // Выделяем память под модель экрана
